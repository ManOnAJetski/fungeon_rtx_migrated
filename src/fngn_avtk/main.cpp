...
public: // v== avk::invokee overrides which will be invoked by the framework ==v
	...
	void update() override
	{
		...
		if (++counter == 4) {
			...
			printf("Time from init to fourth frame: %ld min, %ld sec %lf ms\n", (long)int_min, (long)(int_sec - static_cast<decltype(int_sec)>(int_min) * 60), fp_ms - 1000.0 * static_cast<double>(int_sec));
		}
...
int main() // <== Starting point ==
{
	...
	try {
		...
		auto mainWnd = avk::context().create_window("fngn_atvk_hello");
		...
		mainWnd->set_presentaton_mode(avk::presentation_mode::relaxed_fifo);
		mainWnd->set_number_of_concurrent_frames(2u);
		mainWnd->open();
		...
		auto composition = configure_and_compose(
			avk::application_name("fngn_atvk_hello"),
